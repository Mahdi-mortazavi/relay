...
| **That the stale-build gate works on a phone** (`scripts/device-verify.sh`, 2026-09-21) | Verified without a device: it parses, exits 1 with nothing attached, exits 1 on a missing file, refuses when two devices are present, and its hash comparison separates identical from one-byte-different input | **PARTIALLY VERIFIED.** The half that needs hardware is untested: whether `adb shell sha256sum $(pm path io.relay.app)` returns a hash on a real phone, and whether the mismatch branch exits 2. One run settles both: `scripts/device-verify.sh <the apk you installed>`, then again against a *different* APK, which must exit 2 |
| **That Relay really does stand down when a repository installed it** (`InstallSource`, 2.9.0) | The JVM suite covers the decision — sideload, each listed repository client, and the fail-safe direction for an installer nobody listed. What it cannot cover is the platform call: whether `getInstallSourceInfo` returns what this expects on a real device, and therefore whether the check is actually skipped | **UNVERIFIED — and it is asserted to a third party.** The IzzyOnDroid request in [`docs/store/`](store/izzyondroid-request.md) tells them the copy they serve never fetches a binary. One command settles it, and it does not need F-Droid installed: `adb install -i org.fdroid.fdroid -r Relay-android-arm64-v8a.apk`, launch, and the activity log must read `Updates are the installer's job, not ours installer=org.fdroid.fdroid`. Reinstall without `-i` afterwards and the update banner must come back |
| **That the proxy scan finds a real VPN's port on a real phone** (`ProxyScan`, 2.8.8) | The JVM suite drives the probe against fake servers over real loopback sockets — granted, refused, auth-demanding, accepts-and-never-answers, nothing listening — so the protocol and the refusals are covered. What CI has no copy of is a phone with a VPN client on it | **PARTIALLY VERIFIED.** The exact byte sequence `ProxyScan` sends was mirrored in Python and run against Oblivion's own port through `adb forward` on 2026-09-16: `1819` → granted, bound `127.0.0.1:33311`; `1820` → read timeout, correctly refused. **The Kotlin itself has not run on the phone**, so what is unproven is the app reaching loopback from its own UID and the line appearing under the field |
